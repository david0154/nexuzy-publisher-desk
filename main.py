...
Full restoration with Research Writer AI properly integrated
"""
...
COLORS = {
    ...
    'hover': '#5dade2',
    'active': '#2980b9',
    'research': '#9b59b6'  # Purple for Research Writer
}
...
MODEL_CONFIGS = {
    ...
    'research_writer': {
        'display_name': 'David AI Research Writer',
        'size': '4.5GB',
        'purpose': 'Advanced Research & Long-form Articles',
...
class NexuzyPublisherApp(tk.Tk):
    ...
    def show_research_writer(self):
        """Research Writer - Advanced journalist tool for deep research articles"""
        self.clear_content()
        ...
        tk.Label(self.content_frame, text="🔬 Research Writer - Advanced AI Journalist", 
        ...
        tk.Label(self.content_frame, text=f"Deep research & long-form articles with fact-checking ({model_status})", 
                font=('Segoe UI', 11), bg=COLORS['white'], fg=COLORS['text_light']).pack(padx=30, pady=5, anchor=tk.W)
        ...
        tk.Label(input_frame, text="Research Topic:", font=('Segoe UI', 12, 'bold'), bg=COLORS['light']).pack(padx=20, pady=10, anchor=tk.W)
        
        topic_frame = tk.Frame(input_frame, bg=COLORS['light'])
        topic_frame.pack(fill=tk.X, padx=20, pady=5)
        
        self.research_topic = tk.Entry(topic_frame, font=('Segoe UI', 11), width=60)
        self.research_topic.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        self.research_topic.insert(0, "Enter your research topic here...")
        
        # Options frame
        options_frame = tk.Frame(input_frame, bg=COLORS['light'])
        options_frame.pack(fill=tk.X, padx=20, pady=10)
        
        tk.Label(options_frame, text="Article Length:", bg=COLORS['light']).pack(side=tk.LEFT, padx=5)
        self.research_length = ttk.Combobox(options_frame, values=['Short (500-800 words)', 'Medium (1000-1500 words)', 'Long (2000-3000 words)', 'Deep Dive (3000+ words)'], state='readonly', width=25)
        self.research_length.current(2)  # Default to Long
        self.research_length.pack(side=tk.LEFT, padx=5)
        
        tk.Label(options_frame, text="Style:", bg=COLORS['light']).pack(side=tk.LEFT, padx=(20, 5))
        self.research_style = ttk.Combobox(options_frame, values=['Investigative', 'Academic', 'News Report', 'Feature Story', 'Opinion Piece'], state='readonly', width=20)
        self.research_style.current(0)  # Default to Investigative
        self.research_style.pack(side=tk.LEFT, padx=5)
        
        # Generate button
        ModernButton(input_frame, "🔬 Start Deep Research & Write", self.start_research_writing, 'research').pack(padx=20, pady=15)
        
        # Progress indicator
        self.research_progress = tk.Label(self.content_frame, text="", font=('Segoe UI', 10), bg=COLORS['white'], fg=COLORS['research'])
        self.research_progress.pack(padx=30, pady=5)
        
        # Output Section
        output_frame = tk.Frame(self.content_frame, bg=COLORS['white'])
        output_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=10)
        
        tk.Label(output_frame, text="📝 Research Article Output", font=('Segoe UI', 14, 'bold'), bg=COLORS['white']).pack(anchor=tk.W, pady=10)
        
        # Article preview with scrollbar
        text_container = tk.Frame(output_frame, bg=COLORS['white'])
        text_container.pack(fill=tk.BOTH, expand=True)
        
        scrollbar = tk.Scrollbar(text_container)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.research_output = scrolledtext.ScrolledText(text_container, font=('Segoe UI', 10), 
                                                         wrap=tk.WORD, yscrollcommand=scrollbar.set, height=20)
        self.research_output.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.config(command=self.research_output.yview)
        self.research_output.insert(tk.END, "Your research article will appear here...\n\nThe Research Writer will:\n• Analyze the topic deeply\n• Gather facts from multiple sources\n• Verify information accuracy\n• Generate comprehensive long-form content\n• Add citations and references\n• Structure with proper sections and flow")
        
        # Action buttons
        action_frame = tk.Frame(self.content_frame, bg=COLORS['white'])
        action_frame.pack(fill=tk.X, padx=30, pady=10)
        
        ModernButton(action_frame, "💾 Save as Draft", self.save_research_draft, 'success').pack(side=tk.LEFT, padx=5)
        ModernButton(action_frame, "📋 Copy to Clipboard", self.copy_research_to_clipboard, 'primary').pack(side=tk.LEFT, padx=5)
        ModernButton(action_frame, "📤 Export as File", self.export_research_article, 'warning').pack(side=tk.LEFT, padx=5)
        ModernButton(action_frame, "🗑️ Clear", self.clear_research_output, 'danger').pack(side=tk.LEFT, padx=5)
    
    def start_research_writing(self):
        """Start the research writing process"""
        topic = self.research_topic.get().strip()
        
        if not topic or topic == "Enter your research topic here...":
            messagebox.showwarning("Warning", "Please enter a valid research topic")
            return
        
        if not self.research_writer:
            messagebox.showerror("Error", "Research Writer module not available. Install required dependencies.")
            return
        
        length = self.research_length.get()
        style = self.research_style.get()
        
        self.update_status(f"Researching: {topic}", 'research')
        self.research_progress.config(text="🔬 Starting deep research... This may take a few minutes.")
        self.research_output.delete('1.0', tk.END)
        self.research_output.insert(tk.END, f"Researching: {topic}\nLength: {length}\nStyle: {style}\n\n⏳ Please wait...")
        
        def research_thread():
            try:
                # Call research writer
                article = self.research_writer.write_research_article(
                    topic=topic,
                    length=length,
                    style=style,
                    workspace_id=self.current_workspace_id
                )
                self.after(0, lambda: self._research_complete(article))
            except Exception as e:
                self.after(0, lambda err=str(e): self._research_error(err))
    ...
    def _research_error(self, error):
        ...
        self.research_progress.config(text=f"❌ Error: {error}")
        self.update_status("Research failed", 'danger')
        messagebox.showerror("Error", f"Research failed:\n{error}")
    
    def save_research_draft(self):
        """Save research article as a draft"""
        if not hasattr(self, 'current_research_article') or not self.current_research_article:
            messagebox.showwarning("Warning", "No research article to save")
            return
        
        article = self.current_research_article
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Research article saved as Draft #{draft_id}")
            self.update_status("Research saved", 'success')
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save:\n{e}")
    
    def copy_research_to_clipboard(self):
        """Copy research article to clipboard"""
        content = self.research_output.get('1.0', tk.END).strip()
        if not content or content == "Your research article will appear here...":
            messagebox.showwarning("Warning", "No content to copy")
            return
        
        self.clipboard_clear()
        self.clipboard_append(content)
        messagebox.showinfo("Copied", "Research article copied to clipboard!")
    
    def export_research_article(self):
        """Export research article to file"""
        content = self.research_output.get('1.0', tk.END).strip()
        if not content:
        ...
        if filename:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(content)
                messagebox.showinfo("Success", f"Article exported to:\n{filename}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to export:\n{e}")
    
    def clear_research_output(self):
        """Clear research output"""
        if messagebox.askyesno("Confirm", "Clear current research article?"):
            self.research_output.delete('1.0', tk.END)
            self.research_output.insert(tk.END, "Your research article will appear here...")
            self.research_progress.config(text="")
            if hasattr(self, 'current_research_article'):
                delattr(self, 'current_research_article')
            self.update_status("Research cleared", 'primary')
...
def main():
    logger.info("=" * 60)
    logger.info("Starting Nexuzy Publisher Desk - Complete Platform with Research Writer")
    logger.info("=" * 60)
